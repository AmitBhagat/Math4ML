...
**Solution:** 15.
**Explanation:** The L1 norm is the sum of absolute values: $|4| + |-3| + |7| + |1| = 4 + 3 + 7 + 1 = 15$.`,`**Question 2:** Given the vector $x = [1, -2, 2]$, calculate the L2 norm (Euclidean norm).
**Solution:** 3.
**Explanation:** The L2 norm is the square root of the sum of squares: $\\sqrt{1^2 + (-2)^2 + 2^2} = \\sqrt{1 + 4 + 4} = \\sqrt{9} = 3$.`,`**Question 3:** For the vector $x = [7, -1, -4, 6]$, calculate the L-infinity norm (Max norm).
**Solution:** 7.
**Explanation:** The $L_{\\infty}$ norm is the maximum absolute value among all components: $\\max(|7|, |-1|, |-4|, |6|) = 7$.`,`**Question 4:** If the L2 norm of $x = [6, 8, x_3]$ is 10, find the value of $x_3$.
**Solution:** 0.
**Explanation:** From the L2 definition $\\sqrt{6^2 + 8^2 + x_3^2} = 10$. Squaring both sides: $36 + 64 + x_3^2 = 100$. This gives $100 + x_3^2 = 100$, so $x_3 = 0$.`],code:`import numpy as np
