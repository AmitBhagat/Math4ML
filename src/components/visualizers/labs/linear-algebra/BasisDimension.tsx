...
const BasisDimension = () => {
    ...
    useEffect(() => {
        ...
        // Basis Vectors
        const e1 = board.create('point', [1, 0], { name: 'e1', color: '#E98074', size: 4 });
        const e2 = board.create('point', [0, 1], { name: 'e2', color: '#D8C3A5', size: 4 });
        ...
        const c1 = board.create('slider', [[-6, -5], [-2, -5], [-5, 2, 5]], { name: 'c1', color: '#E98074' });
        const c2 = board.create('slider', [[1, -5], [5, -5], [-5, 3, 5]], { name: 'c2', color: '#D8C3A5' });

        const v = board.create('point', [
            () => c1.Value() * e1.X() + c2.Value() * e2.X(),
            () => c1.Value() * e1.Y() + c2.Value() * e2.Y()
        ], { name: 'v = c_1 e_1 + c_2 e_2', color: '#8E9775', size: 4 });

        board.create('arrow', [[0, 0], v], { strokeColor: '#8E9775', strokeWidth: 3 });

        // Helper components
        board.create('segment', [
            () => c1.Value() * e1.X(), () => c1.Value() * e1.Y(),
            v
        ], { strokeColor: '#D8C3A5', dash: 2 });

        // MathJax: Linear Independence Check
        board.create('text', [-6.5, 6, () => {
            const det = e1.X() * e2.Y() - e1.Y() * e2.X();
            const independent = Math.abs(det) > 0.1;
            const status = independent ? "\\text{Linearly Independent (Basis for } \\mathbb{R}^2)" : "\\text{Dependent (Not a Basis for } \\mathbb{R}^2)";
            return `\\[\\text{Basis Status: } ${status}\\]`;
        }], { fontSize: 16 });
