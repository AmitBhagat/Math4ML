  ...
  sections: [
    {
      ...
      details: ["The derivative measures instantaneous rate of change â€” the slope of the tangent line.", "In ML, it tells how the loss changes when parameters are adjusted."],
      contentSections: [
        {
          ...
          paragraphs: [
            ...
            "Formally: f'(x) = lim(h â†’ 0) [f(x+h) - f(x)] / h"
          ]
        },
        {
          heading: "Key Derivative Rules",
          paragraphs: [
            "Power Rule: d/dx[xâ¿] = nÂ·xâ¿â»Â¹. Example: d/dx[xÂ³] = 3xÂ²",
            "Exponential: d/dx[eË£] = eË£ â€” unique property of e.",
        ...
        },
        {
          ...
          paragraphs: [
            ...
            "Differentiation tells us the gradient of the loss â€” the direction in which the loss increases. We then move in the opposite direction to minimize it.",
    ...
    },
    {
      ...
      formula: "âˆ‚f/âˆ‚xáµ¢",
      details: ["âˆ‚f/âˆ‚x: differentiate f with respect to x, treating all other variables as constants.", "Essential for training models with thousands of parameters simultaneously."],
      contentSections: [
        {
          ...
          paragraphs: [
            ...
            "For a function f(x, y), the partial derivative with respect to x is written âˆ‚f/âˆ‚x and treats y as a constant.",
            "This is essential in ML because loss functions depend on many parameters (weights) simultaneously â€” we need the derivative with respect to each one."
          ]
        },
        {
          ...
          paragraphs: [
            "For f(x, y) = xÂ²y + 3y:",
            "âˆ‚f/âˆ‚x = 2xy  (differentiate with respect to x, y is constant)",
            "âˆ‚f/âˆ‚y = xÂ² + 3  (differentiate with respect to y, x is constant)"
          ],
          code: "# f(x, y) = x^2*y + 3*y\n# Partial derivatives at (x=2, y=3)\nx, y = 2, 3\n\ndf_dx = 2 * x * y  # 2*2*3 = 12\ndf_dy = x**2 + 3   # 4 + 3 = 7\n\nprint(f'âˆ‚f/âˆ‚x at (2,3) = {df_dx}')\nprint(f'âˆ‚f/âˆ‚y at (2,3) = {df_dy}')",
          output: "âˆ‚f/âˆ‚x at (2,3) = 12\nâˆ‚f/âˆ‚y at (2,3) = 7"
        },
        {
          ...
          paragraphs: [
            ...
            "âˆ‚Â²f / (âˆ‚x âˆ‚y) = âˆ‚Â²f / (âˆ‚y âˆ‚x)",
        ...
        },
        {
          ...
          paragraphs: [
            ...
            "Computing all partial derivatives efficiently â€” using the chain rule across layers â€” is what backpropagation does.",
    ...
    },
    {
      ...
      formula: "âˆ‡f = [âˆ‚f/âˆ‚xâ‚, âˆ‚f/âˆ‚xâ‚‚, ..., âˆ‚f/âˆ‚xâ‚™]",
      details: ["âˆ‡f points in the direction of steepest ascent.", "In ML, we move opposite to âˆ‡L (the loss gradient) to minimize the loss."],
      contentSections: [
        {
          heading: "What is the Gradient?",
          paragraphs: [
            "The gradient âˆ‡f (nabla f) is a vector-valued function that collects all the partial derivatives of a scalar function:",
            "âˆ‡f(x) = [âˆ‚f/âˆ‚xâ‚, âˆ‚f/âˆ‚xâ‚‚, ..., âˆ‚f/âˆ‚xâ‚™]",
            "The gradient points in the direction of the steepest increase of f at that point.",
            "Its magnitude ||âˆ‡f|| tells us how steep the slope is â€” a larger magnitude means the function is changing more rapidly."
          ]
        },
        {
          ...
          paragraphs: [
            ...
            "âˆ‡f(x, y) = [2x, 2y]",
            "At point (3, 4): âˆ‡f = [6, 8] â€” pointing away from the origin (the minimum)."
          ],
        ...
        },
        {
          ...
          paragraphs: [
            ...
            "The gradient âˆ‡L tells us in which direction to change w to increase the loss most quickly.",
            "To minimize the loss, we move in the opposite direction: w_new = w - Î± Ã— âˆ‡L(w)",
    ...
    },
    {
      ...
      formula: "Î¸_new = Î¸_old - Î± Â· âˆ‡f(Î¸)",
      details: ["Moves opposite to the gradient to find the minimum.", "Learning rate Î± controls step size â€” critical for convergence."],
      contentSections: [
        {
          heading: "How Gradient Descent Works",
          paragraphs: [
            "Gradient Descent is an iterative optimization algorithm that:",
            "1. Computes the gradient of the loss âˆ‡L(w) at current weights.",
            "2. Updates weights in the opposite direction: w = w - Î± Ã— âˆ‡L(w)",
            "3. Repeats until convergence (gradient â‰ˆ 0 or loss stops improving).",
            "The learning rate Î± controls how large each step is."
          ]
        },
        {
          heading: "The Learning Rate Problem",
          paragraphs: [
            "Too large Î±: the algorithm overshoots the minimum and may diverge.",
            "Too small Î±: convergence is extremely slow â€” takes many more iterations.",
            "Just right Î±: smooth, efficient convergence to the minimum.",
        ...
        },
        {
          ...
          paragraphs: [
            ...
            "Stochastic Gradient Descent (SGD): uses a single random sample each step. Fast but noisy â€” can escape local minima.",
            "Mini-Batch Gradient Descent: uses a small batch (e.g., 32, 64 samples). Best of both worlds â€” most commonly used in deep learning."
          ]
        },
        {
    ...
    },
    {
      id: "ChainRule",
      title: "Chain Rule",
      description: "The chain rule computes the derivative of a composite function. It powers backpropagation â€” the algorithm that trains neural networks.",
      formula: "dz/dx = (dz/dy) Â· (dy/dx)",
      details: ["For z = f(g(x)): dz/dx = f'(g(x)) Â· g'(x).", "Backpropagation applies the chain rule recursively through each neural network layer."],
      contentSections: [
        {
          heading: "What is the Chain Rule?",
          paragraphs: [
            "The Chain Rule computes the derivative of a composite function. If z = f(y) and y = g(x), then:",
            "dz/dx = (dz/dy) Ã— (dy/dx) = f'(g(x)) Ã— g'(x)",
        ...
        },
        {
          ...
          paragraphs: [
            "Find d/dx[sin(xÂ²)]:",
            "Let y = g(x) = xÂ² â†’ dy/dx = 2x",
            "Let z = f(y) = sin(y) â†’ dz/dy = cos(y)",
            "By chain rule: dz/dx = cos(xÂ²) Ã— 2x"
          ],
        ...
        },
        {
          ...
          paragraphs: [
            "A neural network is a composition of many functions: Loss = L(aâ‚™(aâ‚™â‚‹â‚(...aâ‚(x)...)))",
            "To train the network, we need âˆ‚L/âˆ‚wáµ¢ for every weight wáµ¢ in every layer.",
            "Backpropagation applies the chain rule systematically from the output layer backward:",
            "âˆ‚L/âˆ‚wâ‚ = (âˆ‚L/âˆ‚aâ‚™) Ã— (âˆ‚aâ‚™/âˆ‚aâ‚™â‚‹â‚) Ã— ... Ã— (âˆ‚aâ‚‚/âˆ‚aâ‚) Ã— (âˆ‚aâ‚/âˆ‚wâ‚)",
            "Each gradient is computed once and reused â€” making backpropagation computationally efficient."
          ]
        }
      ],
    ...
    },
    {
      ...
      description: "The Jacobian generalizes the gradient to vector-valued functions. The Hessian captures second-order curvature information â€” enabling faster, more powerful optimization.",
      formula: "Háµ¢â±¼ = âˆ‚Â²f / âˆ‚xáµ¢âˆ‚xâ±¼",
      ...
      contentSections: [
        {
          ...
          paragraphs: [
            "For a vector-valued function f: Râ¿ â†’ Ráµ, the Jacobian J is an mÃ—n matrix of partial derivatives:",
            "Jáµ¢â±¼ = âˆ‚fáµ¢ / âˆ‚xâ±¼",
        ...
        },
        {
          ...
          paragraphs: [
            "For a scalar function f: Râ¿ â†’ R, the Hessian H is an nÃ—n matrix of all second-order partial derivatives:",
            "Háµ¢â±¼ = âˆ‚Â²f / (âˆ‚xáµ¢ âˆ‚xâ±¼)",
            "The Hessian describes the curvature of the loss surface at a point:",
            "Positive definite Hessian â†’ local minimum.",
            "Negative definite Hessian â†’ local maximum.",
            "Indefinite Hessian â†’ saddle point (common in deep learning loss surfaces)."
          ],
        ...
        },
        {
          ...
          paragraphs: [
            "Newton's method uses the Hessian for second-order optimization â€” it achieves much faster convergence than gradient descent by accounting for curvature:",
            "w_new = w - Hâ»Â¹ Â· âˆ‡f(w)",
            "Advantage: converges in far fewer iterations than gradient descent.",
            "Disadvantage: computing and inverting H is O(nÂ³) â€” infeasible for large models (millions of parameters).",
      ...
      contentSections: [
        {
          ...
          paragraphs: [
            "Integration is the reverse of differentiation. The definite integral âˆ«â‚áµ‡ f(x)dx computes the accumulated area under the curve f(x) between x = a and x = b.",
            "Geometrically: it's the sum of infinitely many infinitely thin rectangles under the curve.",
            "Fundamental Theorem of Calculus: âˆ«â‚áµ‡ f(x)dx = F(b) - F(a) where F is the antiderivative of f (F' = f)."
          ]
        },
        {
          ...
          output: "âˆ«xÂ² dx from 0 to 3 = 9.0\nArea under N(0,1) = 1.000000"
        },
        {
          ...
          paragraphs: [
            ...
            "AUC (Area Under the ROC Curve) is literally the integral of the ROC curve â€” computed as the area under this curve.",
            "AUC = 1.0: perfect classifier.",
            "AUC = 0.5: random classifier (no discrimination ability).",
            "AUC > 0.8: generally considered a good model.",
            "AUC is threshold-independent â€” it evaluates the model across all possible classification thresholds at once."
          ]
        },
        {
          ...
          paragraphs: [
            ...
            "âˆ«â‚‹âˆž^âˆž f(x) dx = 1",
            "This is why all valid PDFs must integrate to 1 â€” it ensures the total probability of all outcomes is 100%.",
            "The CDF (Cumulative Distribution Function) F(x) = âˆ«â‚‹âˆžË£ f(t) dt gives P(X â‰¤ x)."
          ]
        }
      ],
